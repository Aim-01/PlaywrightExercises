...
test.describe('Тестируем левую часть сайта без кнопок (только поля ввода и чекбокс с радиобаттоном и дропдауном', () => {
  let formPage;
 ...
 test("Выбор из выпадающего списка", async ({ page }) => { //ТРЕТИЙ ТЕСТ ПО ОЧЕРЕДИ
  ...
  await formPage.selectCountry('Russia');
    await expect(formPage.countryDropdown).toContainText('Russia');
